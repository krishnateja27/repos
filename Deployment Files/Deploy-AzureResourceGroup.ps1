...
function Main{
    ...
    try
    {
        ...
        else
        {
             ...
             {
                 Write-Host ("Deleting the webapp with name $websiteName")
                 #Deleting the exisitng webapp
                 Remove-AzureRmWebApp -ResourceGroupName $ResourceGroupName -Name $websiteName
                 Write-Host "Deleted the webapp."
             }
        }
    }
    ...
    try
    {
       ...
       else
       {
             ...
             if($userOption.ToLower() -eq 'y')
             {
                Write-Host "Deleting the functionapp with name" + $functionAppName
    ...
    try
    {
       ...
       else
       {
           ...
           else
           {
                ...
                if($userOption.ToLower() -eq 'y')
                {
                    Write-Host "This is a demo and we are just printing the container name."
                ...
                Write-Host "Have to add the container deployment code."
           }
       }
    }
    catch [Microsoft.Rest.Azure.CloudException]
...
if ($UploadArtifacts) {
  ...
  <#  if ($StorageAccount -eq $null) {
    ...
    while(!(Get-AzureStorageContainer -Name $StorageContainerName -Context $StorageAccount.Context -ErrorAction SilentlyContinue))
    {
        ...
        Write-Host "Trying to create a new container."
